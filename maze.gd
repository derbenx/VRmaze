...
func visualize_path(data, floor_y):
	var p_mat = StandardMaterial3D.new(); p_mat.albedo_color = Color(0, .7, 0); p_mat.shading_mode = StandardMaterial3D.SHADING_MODE_UNSHADED
	...
	var d_mat = StandardMaterial3D.new(); d_mat.albedo_color = Color(.7, 0, 0); d_mat.shading_mode = StandardMaterial3D.SHADING_MODE_UNSHADED
